...
def _make_producer_forward(module, stock_forward, opts):
    ...
    def forward(x, rope_freqs=None, transformer_options={}):
        ...
        try:
            ...
            if vsa:
                raise RuntimeError(
                    "FastH3 VSA execution failed; refusing an incompatible dense fallback"
                ) from exc
            return fallback()
...
def _apply_patch(model, *, tau, start_percent, end_percent, min_tokens,
    ...
    if vsa and (
        not blocks
        or any(not hasattr(block.attn, "to_gate_compress") for block in blocks)
    ):
        raise RuntimeError(
            "FastH3 checkpoint gate_compress layers are missing; install the pinned "
            "Kijai ComfyUI core before loading this model"
        )
...
class SolAttnMiniMax(io.ComfyNode):
    ...
    def execute(cls, model, selection, start_percent, end_percent, min_tokens,
        ...
        return io.NodeOutput(_apply_patch(
...
class H3FastVideoVSA:
    """Legacy wrapper that exposes Kijai's fixed FastH3 VSA recipe."""
    ...
    def patch(self, model):
        if _ck is None:
            raise RuntimeError(f"comfy_kitchen unavailable: {_CK_IMPORT_ERROR}")
        if not hasattr(_ck, "sol_attn"):
            raise RuntimeError("comfy-kitchen 0.2.31+ with sol_attn is required")
        patched = _apply_patch(
            model, tau=1.3,
            start_percent=0.0, end_percent=1.0,
            min_tokens=0, sink_conditioning="exact_kv_and_rows",
            verbose=False, topk_ratio=0.10, vsa=True,
        )
        return (patched,)
